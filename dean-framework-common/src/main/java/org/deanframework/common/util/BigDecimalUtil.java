...
import org.deanframework.common.functional.BigDecimalFunction;
...
public class BigDecimalUtil {
    ...
    public static final Set<Collector.Characteristics> COLLECTOR_SET = Collections.emptySet();
     ...
     * @return
     */
    public static <T> Collector<T, ?, BigDecimal> summing(BigDecimalFunction<? super T> mapper) {
     ...
     * @return
     */
    public static <T> Collector<T, ?, BigDecimal> max(BigDecimalFunction<? super T> mapper) {
        return new CollectorImpl<>(
     ...
     * @return
     */
    public static <T> Collector<T, ?, BigDecimal> min(BigDecimalFunction<? super T> mapper) {
        return new CollectorImpl<>(
     ...
     * @return
     */
    public static <T> Collector<T, ?, BigDecimal> averaging(BigDecimalFunction<? super T> mapper, int newScale, int roundingMode) {
        return new CollectorImpl<>(
                () -> new BigDecimal[]{BigDecimal.ZERO, BigDecimal.ZERO},
                (a, t) -> { a[0] = a[0].add(mapper.applyAsBigDecimal(t)); a[1] = a[1].add(BigDecimal.ONE); },
                (a, b) -> { a[0] = a[0].add(b[0]); return a; },
                a -> a[0].divide(a[1], BigDecimal.ROUND_HALF_UP).setScale(newScale, roundingMode), COLLECTOR_SET);
    }
}
